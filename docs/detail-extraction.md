...
クレジットの抽出は最初の対応役割ラベルから始め、曲紹介・概要・曲の内容に加えて、歌詞・関連動画・コメントでも終了します。HTMLの見出し要素が現れた場合は、音楽配信など見出し名にかかわらずその直前で終了します。クレジットより前にある目次の同名項目は終端にしません。紹介見出しのないページでも、歌詞やコメント内の役割風の文を制作者として取り込みません。未対応の役割ラベルも、全角コロン、前後に空白のある半角コロン、または値が次行なら終端として扱い、その値を直前の対応役割へ混ぜません。名前に含まれる半角コロンは保持します。開いた括弧内の注記は、この役割境界では分割しません。 クレジットではブロック要素と`br`を行区切りとし、リンクや強調要素の前後へ改行を挿入しません。リンクに含まれる名前中の中点・コロンは保持し、リンク外の全角・半角区切り文字で担当者を分けます。

- `page_title`: HTML の `<title>` から Wiki 名を除いたページ名
- `reading`: `曲紹介` 内の `曲名：『...』（...）` から抽出した読み
- `published_year`: 曲詳細ページ内の `YYYY年` タグから推定した公開年
- `credits`: `作詞`, `作曲`, `編曲`, `唄` / `歌`, `絵`, `動画`, `調声`
...
## Tag Listing Integrity

曲一覧は `.cmd_tag` 内の `ul.atwiki-page-list` だけを読みます。各行の曲リンクとタイトル、ページ送りの現在番号・最終番号を検証し、一覧外のナビゲーションを曲やページ数として扱いません。途中ページは先頭ページと同じ行数、最終ページは先頭以下の行数であることを確認します。行数は曲以外の項目を除外する前に比較します。

空一覧、構造不一致、不正リンク、現在ページの不一致、取得中の最終ページ変化、途中の行数減少、同一ページ内・ページ間の曲URL重複は取得失敗です。既存DBを維持し、原文と取得ログを確認してから再実行します。更新中のWikiでページがずれた場合も、不完全な集合を採用せず停止します。

これは応答とページ送りの整合性検査です。Wiki側が一貫した形式で情報を掲載していない場合、その情報の存在までは証明できません。取得元の掲載内容、取得時点、前回DBとの差分・品質ゲートを併せて確認してください。実サイトの取得検証と、保存済み候補DBへの反映は別の作業です。
